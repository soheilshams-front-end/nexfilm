...
export function EpisodeSelector({
  ...
  const [seasonId, setSeasonId] = useState(seasons[0]?.id ?? 1)
        ...
        {season.episodes.map((ep) => (
          <li key={ep.id} className="min-w-0">
            <Link
              ...
              className={cn(
                'group grid w-full min-w-0 items-start transition-colors hover:bg-white/[0.03]',
                compact
                  ? 'grid-cols-[auto_minmax(0,1fr)] gap-x-2.5 gap-y-1 py-2.5'
                  : 'grid-cols-[auto_auto_minmax(0,1fr)] gap-x-2 gap-y-1 py-3 sm:gap-x-3 sm:py-3.5 md:gap-x-4 md:py-4',
              )}
            >
              {/* Episode index — hide in compact to save width */}
              {!compact ? (
